...
export default function RankingPage() {
  ...
  const { t } = useI18n();
  const udocs = args.udocs || args.ulist || [];
  ...
  return (
    <Stack gap="lg">
      ...
      <Paper withBorder>
        <Table striped>
          <Table.Thead>
            <Table.Tr><Table.Th>#</Table.Th><Table.Th>{t('User')}</Table.Th><Table.Th>{t('RP')}</Table.Th><Table.Th>{t('Accepted')}</Table.Th></Table.Tr>
          </Table.Thead>
          <Table.Tbody>
            {udocs.map((u: any, i: number) => (
              <Table.Tr key={u._id}>
                <Table.Td>{(page - 1) * pageSize + i + 1}</Table.Td>
                <Table.Td>
                ...
                <Table.Td><Text size="sm">{Math.round(Number(u.rp) || 0)}</Text></Table.Td>
